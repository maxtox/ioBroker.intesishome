...
module.exports = function (grunt) {
    ...
    var author    = grunt.option('author') || 'Maxtox';
    var email     = grunt.option('email')  || 'maxtox@gmx.de';
    var fs        = require('fs');
    ...
    if (process.argv[2] === 'rename') {
        ...
        if (!newname) {
            console.log('Please write the new intesishome name, like: "grunt rename --name=mywidgetset" --author="Author Name"');
            process.exit();
        }
        ...
        if (fs.existsSync(__dirname + '/admin/intesishome.png')) {
            fs.renameSync(__dirname + '/admin/intesishome.png',              __dirname + '/admin/' + newname + '.png');
        }
        if (fs.existsSync(__dirname + '/widgets/intesishome.html')) {
            fs.renameSync(__dirname + '/widgets/intesishome.html',           __dirname + '/widgets/' + newname + '.html');
        }
        if (fs.existsSync(__dirname + '/widgets/intesishome/js/intesishome.js')) {
            fs.renameSync(__dirname + '/widgets/intesishome/js/intesishome.js', __dirname + '/widgets/intesishome/js/' + newname + '.js');
        }
        if (fs.existsSync(__dirname + '/widgets/intesishome')) {
            fs.renameSync(__dirname + '/widgets/intesishome',                __dirname + '/widgets/' + newname);
        }
    }
            ...
            name: {
                options: {
                    patterns: [
                        ...
                        {
                            match: /Intesishome/g,
                            replacement: newname ? (newname[0].toUpperCase() + newname.substring(1)) : 'Intesishome'
                        },
                        {
                            match: /Maxtox/g,
                            replacement: author
