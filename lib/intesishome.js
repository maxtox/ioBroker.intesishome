...
function setSetPoint(client, deviceId, value) {
    value = parseFloat(value);
    if (isNaN(value)) {
        client.__log.warn('Invalid power value: ' + value);
        return;
    }
    sendCommand(client, deviceId, COMMAND_MAP.setpoint, value);
}

function setFanSpeed(client, deviceId, value) {
    ...
    if (typeof value === 'string') {
        f = findValue(INTESIS_MAP[COMMAND_MAP.fan_speed].native.states);
        if (f === null) {
            client.__log.warn('Invalid power value: ' + value);
            return;
        }
    ...
    sendCommand(client, deviceId, COMMAND_MAP.fan_speed, value);
}

function setPower(client, deviceId, value) {
    let f = parseInt(value, 10);
    if (f.toString() === value.toString()) {
        value = f;
    }
    ...
    if (typeof value === 'string') {
        f = findValue(INTESIS_MAP[COMMAND_MAP.power].native.states);
        if (f === null) {
            client.__log.warn('Invalid power value: ' + value);
            return;
        }
    ...
    sendCommand(client, deviceId, COMMAND_MAP.power, value);
}
...
module.exports.setSetPoint      = setSetPoint;
